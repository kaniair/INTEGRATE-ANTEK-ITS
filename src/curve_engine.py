...
def check_pump_performance(equipment_id: str,
                            flow: float,
                            head: float,
                            power: float,
                            npsha: float,
                            curves: dict) -> PumpStatus:
    """
    ...
    if npsha <= npshr:
        ...
        zone = PumpZone.AOR
        alerts.append(f"⚠️ PERINGATAN: NPSHa ({npsha:.1f}) mendekati NPSHr ({npshr:.1f}) — Risiko kavitasi")
    elif flow < aor_min:
        zone = PumpZone.MINIMUM_FLOW
        alerts.append(f"⛔ KRITIS: Flow ({flow:.1f}) di bawah minimum flow AOR ({aor_min:.1f})")
    elif flow > aor_max:
        zone = PumpZone.AOR
        alerts.append(f"⚠️ PERINGATAN: Flow ({flow:.1f}) melebihi AOR maksimum ({aor_max:.1f})")
...
def check_compressor_performance(equipment_id: str,
    ...
    surge_margin = (inlet_flow - surge_flow) / surge_flow * 100 if surge_flow > 0 else 100

    # ── Zone classification ──
    if inlet_flow <= surge_flow:
        zone = CompressorZone.SURGE
        alerts.append(f"⛔ BAHAYA: Kompresor dalam Surge Zone! Flow={inlet_flow:.1f} ≤ Surge Flow={surge_flow:.1f} MMSCFD")
        alerts.append("🚨 Tindakan segera: Hubungi engineer lapangan, buka Anti-Surge Valve!")
    elif inlet_flow <= protection_flow:
        zone = CompressorZone.PROTECTION
        alerts.append(f"⚠️ PERINGATAN: Mendekati surge line. Surge Margin={surge_margin:.1f}% (minimum 10%)")
        alerts.append("Pantau tren flow dan siapkan intervensi operasi")
    else:
        zone = CompressorZone.OPTIMAL
        alerts.append(f"✅ Normal: Kompresor di zona optimal. Surge Margin={surge_margin:.1f}%")
